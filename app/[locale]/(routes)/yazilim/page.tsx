...
import { BrainCircuit, Cpu, Database, Network, Fingerprint, Microscope, Code, Film, Gamepad2, Camera, Eye, Car, User, OctagonIcon as Polygon, MessageSquare, Smile, BotIcon as Robot, Lock, Image, Languages, MicIcon as Microphone, AlertTriangle, Flame, Wind, Zap, Cog, Monitor } from 'lucide-react'
...
export default function Software() {
  ...
  const aiCapabilities = [
    { name: "Yüz Tanıma Algoritması", icon: User },
    { name: "Araç Tanıma Algoritması", icon: Car },
    { name: "İnsan Tanıma Algoritması", icon: User },
    { name: "Poligon/Çizgi İçi/Gerisi Sayım Algoritması", icon: Polygon },
    { name: "RAG Platformu için LLM Algoritmaları", icon: BrainCircuit },
    { name: "Müşteri Hizmetleri Algoritması", icon: MessageSquare },
    { name: "Duygu Algılama Uygulaması", icon: Smile },
    { name: "Dijital Asistan Projesi", icon: Robot },
    { name: "Sınırlı Geçiş Algoritması", icon: Lock },
    { name: "Görüntü İyileştirme Algoritması", icon: Image },
    { name: "OCR (Optik Karakter Tanıma) Algoritması", icon: Eye },
    { name: "Speech-to-Text (Metin) Algoritması", icon: Microphone },
    { name: "Dil Çeviri Algoritması", icon: Languages },
    { name: "Dublaj Algoritması", icon: Microphone },
    { name: "Plaka tanıma Algoritması", icon: Car },
    { name: "Yasak Alan İhlali Algoritması", icon: AlertTriangle },
    { name: "Sigara Kullanım Tespit Algoritması", icon: AlertTriangle },
    { name: "Özel Nesne Tanıma Algoritması", icon: Eye },
    { name: "Sanal Turnike Sayaç Algoritması", icon: Lock },
    { name: "ISG Uyumluluk Tespit Algoritması", icon: AlertTriangle },
    { name: "Acil Sağlık Durumu Tespit Algoritması", icon: AlertTriangle },
    { name: "Yangın Tespit Algoritması", icon: Flame },
    { name: "Duman Tespit Algoritması", icon: Wind },
    { name: "Elektrik Parlama/Kıvılcım Tespit Algoritması", icon: Zap },
    { name: "Sahaya Uyumlu Özelleştirilmiş Algoritmalar", icon: Cog },
    { name: "Kişiselleştirilmiş Yönetici ve Kullanıcı Arayüzü", icon: Monitor },
  ];
  
  const hardware = [
    { name: "Gömülü Sistemler", icon: Cpu },
    { name: "Tensör İşlemci Birimleri", icon: Cpu },
    { name: "Grafik İşlemci Birimleri", icon: Cpu },
    { name: "Açık Kamera Mimarisi", icon: Camera },
    { name: "Elektrooptik Sistemleri (VNIR, SWIR, MWIR, LWIR)", icon: Eye },
  ];

  const software = [
    { name: "Makina Öğrenmesi", icon: BrainCircuit },
    { name: "Derin Öğrenme", icon: BrainCircuit },
    { name: "İmge İşleme", icon: Image },
    { name: "OCR", icon: Eye },
    { name: "Gelişmiş Sp2Txt-Txt2Sp", icon: Microphone },
  ];

  const solutions = [
    { name: "Afet Yönetimi", icon: AlertTriangle },
    { name: "Trafik Yönetimi", icon: Car },
    { name: "Savunma Sanayii", icon: Lock },
    { name: "Sınır ve Perimetre Güvenliği", icon: Lock },
    { name: "Kent ve Saha Güvenliği", icon: Lock },
    { name: "Sağlık", icon: User },
    { name: "Perakende", icon: Database },
    { name: "Bankacılık", icon: Database },
    { name: "Haberleşme", icon: Network },
  ];
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentIndex((prevIndex) => (prevIndex + 1) % aiCapabilities.length);
    }, 4000);
    ...
    <div className="relative min-h-screen bg-gray-900 text-white overflow-hidden">
      ...
      >
        <motion.div className="text-center">
          <AITextEffect text={aiCapabilities[currentIndex].name} />
          <motion.p
            key={currentIndex}
      ...
      <section className="relative py-20 px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols- gap-8">
          {aiCapabilities.map((capability, index) => (
            ...
            >
              <Card className="bg-gray-800 border-gray-700 hover:bg-gray-700 transition-all duration-300 h-full">
                <CardContent className="p-6 flex flex-col items-center">
                  <capability.icon className="w-12 h-12 mb-4 text-blue-400" />
                  <h3 className="text-lg font-semibold text-center text-white">{capability.name}</h3>
                </CardContent>
              </Card>
            </motion.div>
          ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
          {hardware.map((item, index) => (
            <motion.div
              ...
              transition={{ duration: 0.6, delay: index * 0.1 }}
            >
              <Card className="bg-gray-700 border-gray-600 hover:bg-gray-600 transition-all duration-300 h-full">
                <CardContent className="p-6 flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {solutions.map((solution, index) => (
            ...
            >
              <Card className="bg-gray-700 border-gray-600 hover:bg-gray-600 transition-all duration-300 h-full">
                <CardContent className="p-6 flex flex-col items-center">
                  <solution.icon className="w-12 h-12 mb-4 text-green-400" />
                  <h3 className="text-lg font-semibold text-center text-white">{solution.name}</h3>
                </CardContent>
              </Card>
            </motion.div>
          ))}
